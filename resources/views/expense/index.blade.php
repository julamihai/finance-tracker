...
@section('content')
    ...
    <div class="max-w-md mx-auto ">
        <form action="{{ route('expense.store') }}" method="POST" class="bg-white shadow-md rounded px-8 pt-6 pb-8 mb-4">
            @csrf

            <div class="mb-4">
                <label for="title" class="block text-gray-700 text-sm font-bold mb-2">Expense</label>
            ...
            <table class="min-w-full divide-y divide-gray-200">
                ...
                @foreach($expenses as $expense)
                    <tr>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">{{ $expense->category ? strtoupper($expense->category->title) : 'No category' }}</td>
                        <td class="px-6 py-4 whitespace-nowrap">
                            <a href="{{ route('expense.edit', ['id' => $expense->id]) }}" class="text-green-600 hover:underline">
                                Edit
                            </a>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap">
                            <a href="{{ route('expense.destroy', ['id' => $expense->id]) }}" class="text-red-600 hover:underline">
                                Delete
                            </a>
                        </td>
                    </tr>
